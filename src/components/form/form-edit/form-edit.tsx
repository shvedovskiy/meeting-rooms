...
export const FormEdit: FC<Props> = ({
  ...
  const [updateEvent, { loading: updating }] = useMutation<UpdateEventMutation>(UPDATE, {
  ...
  const [removeEvent, { loading: removing }] = useMutation<RemoveEventMutation>(REMOVE, {
  ...
  function handleFormSubmit(formValues: CreatedEvent) {
    const diff = compareFormStates(formValues as FormFields, initialValues as FormEvent);
